...
import {getSessionTokenByPlayerToken, setPlayerTimers, getPlayerByToken} from "../sessionService.js";
...
export function handleNewChatMessage(socketId: string, message: Message) {
    ...
    // ── Security: always overwrite client-supplied name with server-authoritative name ──
...
function handleCommand(command: string, socketId: string) {
    ...
    if (!commandKey) {
        handleCommandError(socketId, "unknown command — try /ask, /estimate, or /image");
        return;
    }
...
function handleCommandError(socketId: string, error: string) {
    sendAiCommandResponse(socketId, error);
}
